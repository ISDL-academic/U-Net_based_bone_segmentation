...
class Reporter:
    ...
    def create_dirs(self):
        ...
        os.makedirs(self._image_test_dir,exist_ok=True)
    ...
    def save_params(filename, parser):
        ...
        parameters.append("Model:" + str(parser.model))
        output = "\n".join(parameters)

        with open(filename, mode='w') as f:
            f.write(output)

    def save_image(self, train, test, epoch, idx_train, idx_test):
        file_name_train = str(idx_train)+ "_" +self.IMAGE_PREFIX + str(epoch) + self.IMAGE_EXTENSION
        file_name_test = str(idx_test)+ "_" +self.IMAGE_PREFIX + str(epoch) + self.IMAGE_EXTENSION
        train_filename = os.path.join(self._image_train_dir, file_name_train)
        test_filename = os.path.join(self._image_test_dir, file_name_test)
        train.save(train_filename)
        test.save(test_filename)

    def save_image_from_ndarray(self, train_set, test_set, palette, epoch, idx_train, idx_test, index_void=None):
        ...
        #print("In reporter  train_set[0] IS",train_set[0][:,:,0])
        #exit()
        test_image = Reporter.get_imageset(test_set[0], test_set[1], test_set[2], palette, index_void)
        self.save_image(train_image, test_image, epoch, idx_train, idx_test)
