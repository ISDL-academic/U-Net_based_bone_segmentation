...
class ImageAugmenter:
    ...
    def __init__(self, size, class_count):
        ...
        self._ph_original = tf.compat.v1.placeholder(tf.float32, [size[0], size[1], 3])
        self._ph_segmented = tf.compat.v1.placeholder(tf.float32, [size[0], size[1], class_count])
        self._operation = {}
        self.init_graph()
    ...
    # 回転
    def flip(self):
        image_out_index = tf.argmax(input=self._ph_segmented, axis=2)
